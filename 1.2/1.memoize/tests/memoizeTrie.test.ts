...
import { memoizeTrie } from "../memoize";
...
describe("memoize timing behavior", () => {
  it("isPrime: second call hits cache and runs under 0.01 ms", () => {
    const memoIsPrime = memoizeTrie(isPrime);
    ...
    console.log(`[memoizeTrie:isPrime] firstMs=${firstMs.toFixed(3)}ms secondMs=${secondMs.toFixed(3)}ms speedup=${speedupStr}`);
  ...
  it("computeOnce via memoize: second call hits cache and runs under 0.01 ms", () => {
    ...
    const memoOnce = memoizeTrie(computeOnce);
    ...
    const speedup = secondMs > 0 ? firstMs / secondMs : Infinity;
    const speedupStr = Number.isFinite(speedup) ? `${speedup.toFixed(1)}x` : "∞x";
    console.log(`[memoizeTrie:computeOnce] firstMs=${firstMs.toFixed(3)}ms secondMs=${secondMs.toFixed(3)}ms speedup=${speedupStr}`);
